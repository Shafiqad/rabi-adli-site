 ...
 * soft landing). Closer to how a hand actually moves than a flat ease.  */
...
/**
 * Premium signature-draw preloader using real SVG path animation.
 *
 * The wordmark path is drawn via `stroke-dashoffset` — the outline emerges
 * progressively as if a pen is writing it. Once the stroke completes, the
 * fill crossfades in and the stroke fades out, producing a calligraphic
 * "outline → ink" effect. A small spaced-out "Rabi Adli" label then fades
 * in beneath, the panel holds briefly, and finally fades + blurs away to
 * reveal the hero behind it.
 *
...
export function Preloader({ oncePerSession = false }: PreloaderProps) {
  ...
  // Measure the total path length once on mount — needed for the
  ...
  useEffect(() => {
    ...
    const drawDuration = prefersReduced ? 350 : 2300;
    const holdAfterDraw = prefersReduced ? 200 : 850;
      ...
      {visible && (
        ...
        >
          ...
          <div className="relative flex flex-col items-center">
            <div
              className="relative w-[min(360px,82vw)] sm:w-[min(440px,75vw)] md:w-[min(520px,55vw)]"
              style={{
            ...
            >
              ...
              >
                {/* Stroke layer — outline emerges during the draw */}
                <path
                  ref={pathRef}
                  d={SIGNATURE_PATH}
                  fill={signatureDone ? "#F5F5F0" : "transparent"}
                  fillRule="evenodd"
                  stroke="#F5F5F0"
                  strokeWidth={signatureDone ? 0 : 0.5}
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  style={{
                    strokeDasharray: pathLength || 4000,
                    strokeDashoffset:
                      (pathLength || 4000) * (1 - reveal),
                    transition:
                      "fill 700ms cubic-bezier(0.22,1,0.36,1), stroke-width 700ms cubic-bezier(0.22,1,0.36,1)",
                  }}
                />
              </svg>
            </div>
